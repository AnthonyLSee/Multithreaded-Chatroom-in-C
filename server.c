...
{
	/* commandFind() 
	 * - Used to find specific user commands
	 * - Searches for specific text 
	 * - Going to be used in client thread 
	 */
...
{
	/* chathistoryAdd()
	 * - Adds all text from users into a text file, acting as its history.
	 */
...
{	
	/* printList()
	 * - Prints out the usernames in the chatroom
	 */
	 
	int i;
	printf("Current User List_________\n");
	for(i = 0;i<structListSize;i++)
	{
		printf("\x1b[2;32;41mUSER: %s,\x1b[0;0m",userStructList[i]->username);
	}
	printf("\n_________Current User List\n");
	printf("\n");
}
...
{
	/* returnList()
	 * - Used to give a list of all usernames to whoever calls @who
	 */
...
int usernameCheck(char* username)
{
	 ...
	 *    - Empty Username, Username too long, etc
	 */
	...
	for(i = 0; i <= structListSize-1; i++) 		//Compares all Usernames, make sure no duplicates
	{
...
int usernameAdd(User incomingStruct) // NOTE: usernameCheck() ran before this
{
	 ...
	 * - usernameCheck() ran before this, data already clean.
	 */
...
int usernameRemove(User  incomingStruct)
{
	 ...
	 * - Makes sure user is actually there.
	 */
...
{
	/* writeToAllUsers()
	 * - Uses the socket descriptor of the user to send a message to ALL users.
	 * - Forloop + SD + write()
	 * - Status : See what the server wants to do. 
	 *    - EXIT : Will send out if user DC
	 *	  - WRITETOALL : Will write to ALL.
	 */
	
	/*Create/edit the buffer*/
	int i;
	if (strcmp(status,EXIT) == 0) 
		sprintf(buffer,"\x1b[2;32;41mUser: %s disconnected\x1b[0m\n",userRequestStruct->username);
	else if(strcmp(status,WRITETOALL) == 0) 
		sprintf(buffer,"%s%s:%s\x1b[0m",userRequestStruct->color, userRequestStruct->username ,request); //make text to send back
	else return;
...
{
	/* client_session_thread()
	 * - Thread for EACH client.
	 * - Username Validation
	 * - Reads texts / groupchat
	 * - Sends message
	 */	
